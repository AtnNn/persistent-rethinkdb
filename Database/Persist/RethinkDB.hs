-- | Use persistent-rethinkdb the same way you would use other persistent
-- libraries and refer to the general persistent documentation.
--
-- RethinkDB is a schema-less database.
-- The RethinkDB Persistent backend does not help perform migrations.
...
{-# LANGUAGE GADTs #-}
module Database.Persist.RethinkDB
    (
    -- * using connections
    , withConnection
    , createRethinkDBPool
...
instance HasPersistBackend R.RethinkDBHandle R.RethinkDBHandle where
    persistBackend = id

recordTypeFromKey :: Key record -> record
recordTypeFromKey _ = error "recordTypeFromKey"

data Connection = Connection R.RethinkDBHandle
type ConnectionPool = Pool.Pool Connection

-- | ToPathPiece is used to convert a key to/from text
instance PathPiece (BackendKey R.RethinkDBHandle) where
    toPathPiece = todo
    fromPathPiece keyText = todo

instance Sql.PersistFieldSql (BackendKey RethinkDBHandle) where
...
withConnection mc = todo -- withMongoDBPool (mgDatabase mc) (T.unpack $ mgHost mc) (mgPort mc) (mgAuth mc) (mgPoolStripes mc) (mgStripeConnections mc) (mgConnectionIdleTime mc)
...
instance PersistStore R.RethinkDBHandle where
    newtype BackendKey R.RethinkDBHandle = RethinkDBKey { unRethinkDBKey :: R.Datum }
...
instance R.ToDatum PersistValue where
  val (PersistInt64 x)   = R.Number $ fromIntegral x
  val (PersistText x)    = R.String x
  val (PersistDouble x)  = R.Number x
  val (PersistBool x)    = R.Bool x
  val (PersistUTCTime x) = R.Time $ utcToZonedTime x
...
-- | Information required to connect to a RethinkDB database
data RethinkDBConf = RethinkDBConf
    { rdbDatabase :: Text
    , rdbHost     :: Text
    , rdbPort     :: Integer
    , rdbAuth      :: Maybe String
    } deriving Show
